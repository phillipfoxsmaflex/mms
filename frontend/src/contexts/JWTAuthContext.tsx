...
interface AuthContextValue extends AuthState {
  ...
  ) => Promise<{ message: string; success: boolean; token?: string; requiresEmailVerification: boolean }>;
  getInfos: () => void;
...
const AuthContext = createContext<AuthContextValue>({
  ...
  register: () => Promise.resolve({ success: false, message: null, requiresEmailVerification: false }),
  ...
  uploadCustomLogo: () => Promise.resolve(),
...
export const AuthProvider: FC<AuthProviderProps> = (props) => {
  ...
  const login = async (email: string, password: string): Promise<void> => {
    console.log('LOGIN: Sending request with email:', email);
  ...
  const register = async (
    ...
    console.log('[REGISTER DEBUG] Making signup request with values:', values);
    try {
      const response = await api.post<{ message: string; success: boolean; token?: string; requiresEmailVerification: boolean }>(
        'auth/signup',
        {
          ...values,
          utmParams: {
      ...
      console.log('[REGISTER DEBUG] Received response:', response);
      console.log('[REGISTER DEBUG] Response type:', typeof response);
      console.log('[REGISTER DEBUG] Response keys:', response ? Object.keys(response) : 'null/undefined');
      
      const { message, success, token, requiresEmailVerification } = response;
      console.log('[REGISTER DEBUG] Extracted message:', message, 'success:', success, 'token:', token, 'requiresEmailVerification:', requiresEmailVerification, 'invitationMode:', invitationMode);

      if (requiresEmailVerification || invitationMode) {
        console.log('[REGISTER DEBUG] Email verification required or invitation mode, returning response');
        return response;
      } else if (token) {
        console.log('[REGISTER DEBUG] Calling loginInternal with token');
        try {
          await loginInternal(token);
          console.log('[REGISTER DEBUG] loginInternal completed successfully');
        } catch (loginError) {
          console.error('[REGISTER DEBUG] Error in loginInternal:', loginError);
          throw loginError;
        }
        console.log('[REGISTER DEBUG] Returning response:', response);
        return response;
      } else {
        console.log('[REGISTER DEBUG] No token and no email verification required, returning response as-is');
        return response;
      }
    } catch (error) {
      console.error('[REGISTER DEBUG] Error in register function:', error);
      throw error;
    }
  };
  ...
  const uploadCustomLogo = async (file: File): Promise<void> => {
    ...
    // Update the UI configuration with the new logo URL
    const uiConfiguration = await api.patch<UiConfiguration>(
      `ui-configurations`,
      { customLogo: response.logoUrl }
    );

...
public class AuthController {
    ...
    public EnhancedSignupResponse<UserResponseDTO> signup(@Valid @RequestBody UserSignupRequest user) {
        //TODO: add currentUser especially for disabled email verifications
        EnhancedSignupResponse<OwnUser> response = userService.signup(user);
        UserResponseDTO userResponseDTO = response.getUser() != null ? userMapper.toResponseDto(response.getUser()) : null;
        return new EnhancedSignupResponse<>(response.isSuccess(), response.getMessage(),
                response.getToken(), response.isRequiresEmailVerification(),
                userResponseDTO);
    }
